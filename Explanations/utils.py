...
def preprocess_image(img_path, target_size=(480, 640)):
...
#         with image.load_img(img_location, target_size=target_size) as img:
...
def show_imgwithheat(img_path:str, heatmap, alpha=0.4, return_array=False):
...
#         superimposed_img = heatmap * alpha + img
#         superimposed_img = np.clip(superimposed_img, 0, 255).astype("uint8")
#         superimposed_img = cv2.cvtColor(superimposed_img(img.UMat), cv2.COLOR_BGR2RGB)
#         imgwithheat = Image.fromarray(superimposed_img)  
#         display(imgwithheat)
#         frameSize = (320, 240)
#         images.append(imgwithheat)
